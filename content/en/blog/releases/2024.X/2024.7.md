...
* [Reduce the number license check calls][] in order to reduce the number of calls to the Licence Management Service.
...
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} API Gateway Service][] </nobr>              | 36.0.3.24340  | Yes     | Major       | Potential |       | <!-- Now use Scope instead of ReliableCollectionScope & Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Authorisation Service][]                    | 5.0.2.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Concurrency Management Service][] </nobr>   | 5.0.2.24340   | Yes     | Major       | Potential |       | <!-- Now use Scope instead of ReliableCollectionScope & Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Configuration Management Service][] </nobr> | 4.0.2.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Data Storage Service][] </nobr>             | 6.0.2.24340   | Yes     | Major       | Potential |       | <!-- Now use Scope instead of ReliableCollectionScope & Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Execution Management Service][] </nobr>     | 5.0.3.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Licence Management Service][] </nobr>       | 5.0.3.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Package Management Service][] </nobr>       | 9.0.2.24340   | Yes     | Major       | Potential |       | <!-- Lots of changes including Compression -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Provisioning Service][] </nobr>             | 10.0.3.24340  | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Scheduling Service][] </nobr>               | 5.0.3.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Triggers Service][] </nobr>                 | 7.0.3.24340   | Yes     | Major       | Potential |       | <!-- Moved where the ServiceActivityFactory is instantiated & service now implement IDisposable -->
| <nobr>[{{% ctx %}} Innovation Execution Application][] </nobr>                 | 19.1.2.24340  | Yes     | Major       | Potential |       |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Execution Service][] </nobr>                | 19.1.2.24340  | Yes     | Major       | Potential |       | <!-- Lots of changes -->
| <nobr>[{{% ctx %}} Gateway][Gateway], including [{{% ctx %}} Studio][] </nobr> | 10.33.1.24340 | Yes     | Minor       | No        |       |
| <nobr>[{{% ctx %}} Blocks Package][Blocks] </nobr>                             | 46.0.4.24340  | Yes     | Major       | Potential |       | <!-- Includes OpenAPI block and no way to hide it no? -->
| <nobr>[{{% ctx %}} Interaction Portal][Interaction Portal] </nobr>             | 2.0.0.24340   | Yes     | Major       | Yes       |       | <!-- Newer version of flows will need to be imported -->
| <nobr>{{% ctx %}} App Server Install Scripts </nobr>                           | 5.1.0.24140   | No      | N/A         | No        |       |
| <nobr>{{% ctx %}} Web App Server Install Scripts </nobr>                       | 10.0.0.24330  | Yes     | Major       | No        | Major changes occurred between internal development releases, however, these changes are not breaking between 2024.5 and 2024.7 |
...
All flows that uses the previous `Scope` data type will need to be upgraded using the {{% ctx %}} Flows Upgrader.
