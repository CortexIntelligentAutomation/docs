...
Also related to logs, we have also fixed a bug where [Password redaction not always occurring][].  
\
Scalability has been increase by adding [support of custom names for default Tenant and System][Custom names supported for default Tenant and System] from {{% ctx %}} Gateway.  
[Installing multiple {{% ctx %}} Gateway instances on the same Web App Server][Support for installing multiple {{% ctx %}} Gateway instances on the same Web App Server] is now also supported.  
\
...
\
The [user experience of {{% ctx %}} Gateway][{{% ctx %}} Gateway] has been improvements which includes:
...
The [Engine][] has also been improved by fixing a bug where a [Flow with expression sometimes fails to execute with a KeyNotFoundException][].  
\
[Support for installing multiple {{% ctx %}} Gateway instances on the same Web App Server][] has been added to the installation scripts.  
\
Finally, this [Product Portal][] includes updates related to:

* [Wireless Blocks][], [WirelessReceiverBlockReference][] data type and the [WirelessReceiverBlockNotFoundException][].
* [Execute Data Commmand Block][] and data types
* [Scope][] data type and the [ScopeDefinition][] data type
* [upgrade steps][] from 2024.5 to 2024.7 including upgrade flows steps
* Block icons and screenshots to reflect rebranding
...
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Package Management Service][] </nobr>       | 9.0.2.24340   | Yes     | Major       | Yes             | Major changes occurred between releases, these changes are related to compression of flows related to [Reduce memory usage of the Package Management Service][]. |
...
| <nobr>[{{% ctx %}} Blocks Package][Blocks] </nobr>                             | 46.0.4.24340  | Yes     | Major       | Yes             | Flows will need to be [upgrading flows][upgraded] |
| <nobr>[{{% ctx %}} Interaction Portal][Interaction Portal] </nobr>             | 2.0.0.24340   | Yes     | Major       | Yes             | The Interaction Portal flows will need to be [upgrading flows][upgraded] |
...
[Reduce the number license check calls]: {{< ref "#reduced-the-number-license-check-calls" >}}
...
[Execute Data Commmand Block]: {{< url path="Cortex.Reference.Blocks.Data.ExecuteDataCommand.ExecuteDataCommand.MainDoc" version="2024.7" >}}
