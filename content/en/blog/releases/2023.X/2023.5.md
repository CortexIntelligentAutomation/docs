...
[OAuth2 authorisation has been introduced for running flows in published packages][]. The [HA Platform][] can be configured to authenticate against a single Active Directory server, allowing authorised users to start the a flow execution. Package Management within Gateway can be used to modify access control to any version of a package.
...
Finally, this [Product Portal][] has been rebranded to follow We Are CORTEX branding guidelines, the documentation has also been updated to reflect the [new and upcoming services][HA Platform], and reference documentation has also been added for the [new blocks][New Blocks], as well as improvements to the existing [SSH data types][].
...
| &nbsp; &nbsp; > [{{% ctx %}} API Gateway Service][CORTEX API Gateway Service]                           | 25.3.0.23250 | Yes                  | Major         | [Potential breaking change][Problem Details Replace Exceptions in API Calls] |
...
| &nbsp; &nbsp; > [{{% ctx %}} Execution Management Service][CORTEX Execution Management Service]         | 1.1.0.23250  | Yes                  | First Release | For future use                |
...
The Configuration Management service has been added to the HA Platform, which currently allows managing tenants/systems for deployed {{% ctx %}} Innovation instances.
...
Any changes made to the `Authorisation` settings of a package version are saved to source control, allowing for transparency in when edits have been made. `Authorisation` settings are also persisted when creating a new version and can be modified for each version independently.
...
The Product Portal has been rebranded to follow We Are CORTEX branding guidelines, the documentation has also been updated to reflect the [new and upcoming services][HA Platform], and reference documentation has also been added for the [new blocks][New Blocks], as well as improvements to the existing SSH documentation for the following data types:
...
Responses from [API Gateway][CORTEX API Gateway Service] now return problem details instead of exceptions when an error occurs. Problem Details are now returned in the response body in a machine-readable format, and allows for improved usability when interacting with errors returned APIs.
...
[Http Request block]: {{< url path="Cortex.Reference.Blocks.Http.ExecuteHttpRequest.ExecuteHttpRequest.MainDoc" version="2023.5" >}}
