...
#### Lay foundations for Shared State
Add Tenant/System management API
Reading/Writing Data from reliable collections via APIs

#### HA Flow Debugging
The foundation to allow High-Availability of flow debugging 
Debug via Studio:
  Flow can be executed in debug mode
  API to get all running executions
  Debug commands works
  Parameterisation of partition count
Retrieving Help Links:
  Gateway help link is read from helplink config (not flow debugger)
