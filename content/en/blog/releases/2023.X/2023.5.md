...
#### Expansion of the HA Platform architecture
...
An API is now available to manage tenants/systems, such as:
...
An API has been added to read and write data.
The API currently only support Basic Authorisation.

#### New Authorisation Service

The [Authorisation Service][] has been introduced to the HA Platform, which includes the previously provided Basic Authorisation, but also introducing OAuth2 authentication to the API Gateway of the HA Platform.
...
With the introduction of the [Authorisation Service][], OAuth 2 is now supported. In the first place, it will allow users to set authorisation to a specific package, making each published package access controlled.

The {{% ctx %}} Gateway [Packages][] settings page now allows setting package authorisation.
For that, a new `Authorisation` section has been added to the package definition view, which is visible when clicking on a specific package version, as a tab.

`Authorisation` settings are persisted when creating a new version and can be modified for each version.

An API is now available to manage authorisation to create a new token and refresh the token.

#### Improved the Interaction Portal
...
Various customer feedback has been addressed to improve the installation process of {{% ctx %}} Innovation, such as better checks and errors.

New parameters are now required for the installation of the Application Servers to configure OAuth authorisation.

#### Updated the Product Portal

The Product Portal has been rebranded to follow We Are CORTEX branding guidelines.
The documentation has also been updated to reflect the new and upcoming services.
The [blocks][] help documentation has also been updated for the new [Telnet][] block, as well as SSH documentation for the SshLogs and SshSessionDetails data types.

### Removed Feature

#### Removed intro video from Gateway Setup

The introduction video, which was available when first setting up {{% ctx %}} Gateway, has now been removed.

#### Removed old bundled help

The original help bundles for the preview version of {{% ctx %}} innovation have now been removed.
