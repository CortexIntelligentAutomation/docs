...
* [Configuration Management Service][CORTEX Configuration Management Service] - Allows for management tenants/systems for a deployed {{% ctx %}} Innovation platform
* [Data Storage Service][CORTEX Data Storage Service] - Allows for sharing of data between executions of flows
* [Authorisation Service][CORTEX Authorisation Service] - Allows for OAuth2 authentication within the API Gateway Service (currently only available for the execution of flows in published packages, but will be expanded in future releases)

[OAuth2 Authorisation has been introduced for running flows in published packages][]. The [HA Platform][] can be configured to authenticate against a single AD server, allowing authorised users to start the execution of a flow. Package Management within Gateway can be used to modify access control to any version of a package.
...
discuss about the blocks capabilities, now capable of running flow asynchronously, and execute telnet command, similar
...
Finally, this [Product Portal][] has been rebranded to follow We Are CORTEX branding guidelines, the documentation has also been updated to reflect the [new and upcoming services][HA Platform], and reference documentation has also been added for the [new blocks][New Blocks], as well as improvements to the existing SSH data type reference documentation
