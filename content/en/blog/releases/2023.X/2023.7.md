...
The [Flow Debugger service has been removed][], with the debugging capability moved in to the Execution Service. This means that the debugging of flows can now be highly available if needed, and opens up the possibility of debugging executions in production in the future.

Five [new Blocks][New Blocks] have been added to the {{% ctx %}} Block Packages, adding new functionality to work with:

* Data Storage - Allowing for the creation and deletion of collections within the Data Storage Service, and the capability to read, write and delete data from collections

Two [new Common Properties][New Common Properties] have been added to most blocks within the {{% ctx %}} Block Packages, adding new functionality to work with:

* Semaphore - Allowing for the execution of the block to be limited and controlled by accessing a specific semaphore
* Run As - Allowing for the execution of the block to run as an impersonated user

The API Gateway has been consolidated and improved to return more consistent status codes, problem details, and data.
...
|--------------------------------------------------------------|--------------|----------------------|---------------|--------------------------------------------------------|
| {{% ctx %}} Innovation Core Application                      | 35.1.4.23350 | Yes                  | Major         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} API Gateway Service              | 30.2.3.23350 | Yes                  | Major         | [Potential breaking change][API Gateway Consolidation] |
| &nbsp; &nbsp; > {{% ctx %}} Authorisation Service            | 2.4.4.23350  | Yes                  | Minor         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Concurrency Management Service   | 1.1.3.23350  | Yes                  | First Release |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Configuration Management Service | 1.5.3.23350  | Yes                  | Minor         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Data Storage Service             | 2.1.4.23350  | Yes                  | Major         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Execution Management Service     | 2.3.3.23350  | Yes                  | Minor         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Licence Management Service       | 1.1.3.23350  | Yes                  | Minor         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Package Management Service       | 6.0.4.23350  | Yes                  | Major         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Provisioning Service             | 6.4.3.23350  | Yes                  | Minor         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Scheduling Service               | 2.1.5.23350  | Yes                  | First Release |                                                        |
| {{% ctx %}} Innovation Execution Application                 | 9.0.4.23350  | Yes                  | Major         |                                                        |
| &nbsp; &nbsp; > {{% ctx %}} Execution Service                | 9.0.4.23350  | Yes                  | Major         |                                                        |
| {{% ctx %}} Gateway, including {{% ctx %}} Studio            | 4.5.1.23350  | Yes                  | Major         |                                                        |
| {{% ctx %}} Blocks Package                                   | 39.0.1.23350 | Yes                  | Major         |                                                        |
| {{% ctx %}} Interaction Portal                               |              | Yes                  |               |                                                        |
...
The Concurrency Management service has been added to the HA Platform, which allows for for the execution of blocks to be limited and controlled by access to a specific semaphore.

This can be configured individually for each block, by using the [new common block property][New Common Properties] for semaphores available on most blocks.
...
The Scheduling service has been added to the HA Platform, which allows for scheduled executions of flows within the {{% ctx %}} Innovation platform.

Schedules can be added, or deleted, when managing a version of a package within {{% ctx %}} Gateway.
...
Responses from API Gateway have been consolidated and improved, they now return more consistent status codes, messages and data.

This is [potentially breaking][API Gateway Consolidation] if you were interacting with responses returned by any API call to API Gateway.
...
The Flow Debugger Service has been removed, with the debugging capability moved in to the Execution Service.
...
This is potentially breaking if you were interacting with responses returned by any API call to API Gateway (i.e. a 3rd party system, or a flow using the Execute Http Request block to call API Gateway endpoints). Any occurrence of this will need to be updated to use the new response.
...
If you have installed 2023.5 release and want to upgrade to 2023.7; please request assistance by raising a case in the {{% ctx %}} Service Portal.
