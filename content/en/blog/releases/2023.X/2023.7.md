...
* [Scheduling Service][] - Allows for scheduled executions of flows within the {{% ctx %}} Innovation platform
...
These flows can be configured with a CRON schedule to control when the execution occurs, and a schedule can be created with input variables that are passed in to the execution.
