...
When trying to call an endpoint with the [Execute Http Request][] block in the 2023.5 platform, using Basic Auth, the calls with respond with 401 unauthorised response despite using the same correct credentials.

This issue has been resolved as part of the 2023.7 platform, and Basic Auth can be used to make calls using the [Execute Http Request][] block.

#### DuplicatedBlockIdException prevents running flow
...
This issue has been resolved as part of the 2023.7 platform, and the DuplicatedBlockIdException is no longer thrown when two third parties make a call to start an execution of the same flow at the same time.

#### EncryptedText Parameters in the the Execute PowerShell Script Block Passed the Redacted Value to the Script
...
This issue has been resolved as part of the 2023.7 platform, and the [Execute PowerShell Script][] block can now use [EncryptedText][] values as parameters.
...
This issue has been resolved as part of the 2023.7 platform, input variables with an [EncryptedText][] default value no longer wrapt the input variable with quotes.

#### Cortex Data Types could not be Serialised with TypeNameHandling set to All

When trying to serialise any Cortex [Data Type][] using either [Write Data With Key][] or [Convert Object To Json][] blocks, when TypeNameHandling was set to `TypeNameHandling.All`, subsequent use of any Cortex Data Type would cause any block to throw a JsonSerialisationException.

This issue has been resolved as part of the 2023.7 platform, and the JsonSerialisationException is no longer thrown when trying to serialise any Cortex [Data Type][] using either [Write Data With Key][] or [Convert Object To Json][] blocks, when TypeNameHandling was set to `TypeNameHandling.All`.
...
This issue has been resolved as part of the 2023.7 platform, and the JsonSerialisationException is no longer thrown when trying to deserialise any Cortex [Data Type][] using the [Convert Json To Object][] block, when TypeNameHandling was set to `TypeNameHandling.All`.
