...
2025.3.26111 is the latest [Patch][] of the next generation of {{% ctx %}} and improves on the [2025.3][] release in the following areas:
...
##### Added the ability to safely cancel Data Storage Blocks operations

[Data Storage Blocks][Data Storage Blocks] now support cancellation across all data storage operations, ensuring blocks shut down safely when an execution is stopped or the block execution times out.
...
##### Added the ability to encrypt values in the CORTEX Configuration Portal

You can now securely encrypt parameter values directly when adding them to a container.
A new encryptor tool lets you enter text, encrypt it, and automatically copy the encrypted value to your clipboard.
...
The [{{% ctx %}} Execution Management Service][] now limits the batch size of execution completion details, reducing inter‑service traffic.
...
The retention period is configurable based on the retention that is required.
...
A bug was fixed where making a change on the expression editor was sometimes lost when clicking on another block then clicking back on the block where the change as made.
...
A bug was fixed where deleting a trigger would failed after a restart of the [{{% ctx %}} Triggers Service][].
Affected Components:
