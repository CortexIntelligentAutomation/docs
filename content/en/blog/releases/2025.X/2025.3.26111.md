...
##### Added support for multiple registry keys

Encryption now uses a clear, consistent key‑selection process to avoid silent failures and corrupted decrypted values.
Each encrypted value includes a key identifier, active keys are always prioritised, and retired or compromised keys are no longer used for new encryption.
The system also provides better logging and warnings when outdated or invalid keys are involved.
...
The dependency on Active Directory cmdlets was removed from the `Installation` of [{{% ctx %}} Gateway][Gateway].
...
A bug was fixed where deleting a trigger would silently failed after a restart of the [{{% ctx %}} Triggers Service][].
Affected Components:
...
There are no features removed in the 2025.326111 patch of the {{% ctx %}} Platform.
...
[Secrity - Added Encryption Endpoint to API Gateway Service]
