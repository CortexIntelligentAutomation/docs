...
|--------------------------------------------------------------------------------|---------------|---------|---------------|-----------------|--------|
| <nobr>[{{% ctx %}} Innovation Core Application][] </nobr>                      | 56.1.4.25130  | Yes     | Major         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} API Gateway Service][] </nobr>              | 40.0.5.25130  | Yes     | Major         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Authorisation Service][] </nobr>            | 7.0.18.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Concurrency Management Service][] </nobr>   | 7.3.3.25130   | Yes     | Minor         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Configuration Management Service][] </nobr> | 7.0.18.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Data Storage Service][] </nobr>             | 9.0.30.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Execution Management Service][] </nobr>     | 7.0.25.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Licence Management Service][] </nobr>       | 7.0.18.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Listeners Service][] </nobr>                | 3.0.3.25130   | Yes     | Major         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Monitoring Service][] </nobr>               | 2.6.3.25130   | Yes     | Minor         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Package Management Service][] </nobr>       | 11.0.22.25130 | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Provisioning Service][] </nobr>             | 14.0.6.25130  | Yes     | Major         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Scheduling Service][] </nobr>               | 7.0.28.25130  | Yes     | Patch         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Triggers Service][] </nobr>                 | 11.0.20.25130 | Yes     | Patch         | ---             |        |
| <nobr>[{{% ctx %}} Innovation Execution Application][] </nobr>                 | 25.3.7.25130  | Yes     | Minor         | ---             |        |
| <nobr>&nbsp; &nbsp; > [{{% ctx %}} Execution Service][] </nobr>                | 25.3.7.25130  | Yes     | Minor         | ---             |        |
| <nobr>[{{% ctx %}} Gateway][Gateway], including [{{% ctx %}} Studio][] </nobr> | 12.17.1.25130 | Yes     | Minor         | ---             |        |
| <nobr>[{{% ctx %}} Blocks Package][Blocks] </nobr>                             | 48.3.0.25130  | Yes     | Minor         | ---             |        |
| <nobr>[{{% ctx %}} Interaction Portal][Interaction Portal] </nobr>             | 3.1.0.24470   | Yes     | Minor         | ---             |        |
| <nobr>{{% ctx %}} App Server Install Scripts </nobr>                           | 5.2.9.25120   | Yes     | Patch         | ---             |        |
| <nobr>{{% ctx %}} Web App Server Install Scripts </nobr>                       | 8.1.2.24460   | No      | N/A           | ---             |        |
| <nobr>{{% ctx %}} Upgrade Scripts </nobr>                                      | 1.2.18.25120  | Yes     | Patch         | ---             |        |
| <nobr>{{% ctx %}} Licence Fingerprint Generator </nobr>                        | 4.1.0.24360   | No      | N/A           | ---             |        |
| <nobr>{{% ctx %}} Encryption Key Generator </nobr>                             | 2.0.0.24350   | No      | N/A           | ---             |        |
| <nobr>{{% ctx %}} Encryptor </nobr>                                            | 3.1.0.24330   | No      | N/A           | ---             |        |
| <nobr>{{% ctx %}} Flows Upgrader </nobr>                                       | 2.0.1.24450   | No      | N/A           | ---             |        |
| <nobr>{{% ctx %}} Observability</nobr>                                         | 3.0.0         | No      | N/A           | ---             |        |
...
<!--	API Gateway - Semaphore Controller - Delete all executions for semaphore on a specified scope	-->
<!--	API Gateway - Semaphore Controller - Delete an execution for semaphore on a specified scope	-->
<!--	API Gateway - Semaphore Controller - Get all semaphores for given scope/all scopes	-->
...
<!--	Create Process From BPMN XML - Add titles to workspaces	-->
<!--	Create Process From BPMN XML - Create connections using BPMN XML	-->
<!--	Create Process From BPMN XML - Make created flow able to be run in gateway	-->
<!--	Create Process From BPMN XML - Make sure blocks are on grid	-->
<!--	Create Process From BPMN XML - Space out blocks to avoid overlap	-->
<!--	Create Process From BPMN XML - Support all types of task and sub-processes	-->
<!--	Create Process From BPMN XML - Support all valid BPMN XML	-->
...
<!--	Disaster Recovery - Enable BackUpRestore Service	-->
...
<!--	Execution Management - Refactor StartExecution() method in ExecutionManager.cs	-->
<!--	Service Fabric - MonitoringManager improvements	-->
...
<!--	Security - Introduce Referrer-Policy header for Cortex Interaction Portal	-->
<!--	Security - Introduce Strict-Transport-Security header for Cortex Interaction Portal	-->
<!--	Security - Introduce X-Content-Type-Options header for Cortex Interaction Portal	-->
<!--	Security - Introduce X-Frame-Options header for Cortex Interaction Portal	-->
...
<!--	Upgrade 3rd Party Libraries - Bootstrap library should be updated (Cortex Gateway) + Bootstrap-datetimepicker and AngularJS	-->
<!--	Upgrade 3rd Party Libraries - Bootstrap-datetimepicker library should be updated (Cortex Gateway)	-->
...
<!--	Create unblock script and update docs to include	-->
...
<!-- *** NOT DOCUMENTED -->
<!--	API Gateway - Semaphore Controller - Non-Queueable semaphore should return null	-->
<!--	API Gateway - Semaphore Controller - Stuck Semaphore cannot be removed if using the Current scope	-->
<!--	Azure Security - Issue with value & handling of LDAPAuth credential stored in Library	-->
<!--	Azure to LAB - Cortex.EndToEnd.ViaGateway pipeline running against new LAB VM	-->
<!--	Azure to LAB - Manually create Test VM on Proxmox infrastructure	-->
<!--	Azure to LAB - Review Existing Resources & Functionality	-->
...
<!--	G2 Migration - 2 week proof of concept	-->
<!--	Migration Tool - Suggested Improvements from Projects	-->
<!--	Migration - Enable viewing of Old Block Properties within evolution flow	-->
<!--	Licensing - DotNet flow execution handling not processed when calculating license cost	-->
<!--	Product Portal - Add functionality for a "latest" url	-->
<!--	Security - Support MFA - Endpoint for OpenID Connect Configuration	-->
<!--	Security - Support MFA - POC	-->
<!--	Disaster Recovery - Investigate Repair Manager Service and Deactivate (Restart) options for taking node out of service for patching	-->
<!--	Disaster Recovery - Research what needs to be done	-->
<!--	Installation - Rename LegacyFlowApi to FlowApiProxy	-->
<!--	Operations Overview - cannot scroll when switching between different time period on v-labappdb9.lab.ad.cortex.uk (slow/freeze)	-->
