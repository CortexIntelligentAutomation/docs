...
* The user can now [create a new process from BPMN XML][] within {{% ctx %}} Gateway
* An [Average Duration graph has been added][] to the Operations Overview page on {{% ctx %}} Gateway

All services have had the [default size of logs][] used for [Observability][] reduced to optimise disk usage.

Work has been undertaken to [improve the performance][] of the {{% ctx %}} Execution and {{% ctx %}} Monitoring services.
...
[Security][] has been tightened on the {{% ctx %}} Interaction Portal by:
...
| <nobr>{{% ctx %}} Gateway, including {{% ctx %}} Studio </nobr>            | 12.17.1.25130 | Yes     | Minor       | N/A             |                                                                                                                                                                                                                           |
| <nobr>{{% ctx %}} Blocks Package </nobr>                                   | 48.3.0.25130  | Yes     | Minor       | N/A             |                                                                                                                                                                                                                           |
| <nobr>{{% ctx %}} Interaction Portal </nobr>                               | 3.1.0.24470   | Yes     | Minor       | N/A             |                                                                                                                                                                                                                           |
...
Within {{% ctx %}} Gateway, the user can now create a new process from a template. The currently supported templates are `Blank` and `BPMN XML`.
...
The {{% ctx %}} Gateway's Operations Overview page now includes an `Average Duration` graph, which displays the average flow execution duration, as well as a graphs for the average duration over the specified time period.
...
The following security headers have been introduced to the {{% ctx %}} Interaction Portal:
...
The following sensitive information has been removed from {{% ctx %}} Interaction Portal's HTTP responses:
...
The {{% ctx %}} Gateway's Operations Overview page now includes the following improvements to the user experience:
...
The {{% ctx %}} version is now available on the {{% ctx %}} Gateway logo tooltip.
