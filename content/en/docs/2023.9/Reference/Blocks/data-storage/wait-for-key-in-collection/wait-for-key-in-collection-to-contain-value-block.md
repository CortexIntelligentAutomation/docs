...
Waiting for `"user2"` to contain `{"Domain": "domain", "Username": "user1", "Password": "encryptedPassword"}` in the [Data Storage Collection][] results in the execution progressing as `"user2"` is not contained within the [Data Storage Collection][].
