...
The exception thrown when an issue occurs with a [HTTP request][Request].

## Reasons

### Invalid Request Body

A request body for the [request][Request] was provided when one should not have been provided.
...
The [request][Request] body provided does not match the content type.
...
Ensure that the content type of the body provided matches that which is required for the [request][Request].

### Invalid Request Enum Provided

An [Enum][] value provided for the [request][Request] is invalid, e.g. the [request verb][RequestVerb] or the [request Version][HttpRequestVersion].
