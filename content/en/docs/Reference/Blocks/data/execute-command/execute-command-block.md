...
Selecting all rows and columns that have an Id less than 3 from `Table` using a [QueryCommand][Command Types AnyCommand] results in the variable `($)Result` being updated to the following:
...
A [NonQueryCommand][Command Types NonQueryCommand] is used throughout this example to update data in the data source, however, both an [AnyCommand][Command Types AnyCommand] or [AnyCommands][Command Types AnyCommands] could also be used to the same effect.
...
A [NonQueryCommand][Command Types NonQueryCommand] is used throughout this example to delete data in the data source, however, both an [AnyCommand][Command Types AnyCommand] or [AnyCommands][Command Types AnyCommands] could also be used to the same effect.
...
An [AnyCommands][Command Types AnyCommands] is used throughout this example, as it is the only type of [Command][Command Property] that allows you to execute and return the results of multiple commands.
...
Running the [AnyCommands][Command Types AnyCommands] results in the variable `($)Result` being updated to the following:
...
- [Connection Type][] - shows a selection of supported data sources to pick from, please see [Working with Data Sources][] for more information.
...
| [ArgumentException][] | Thrown when the connectionType within the [Connection Details][Connection Details Property] is not one of the specified [ConnectionType][Connection Type] types (e.g. `(ConnectionType)100`). |
...
| | Thrown when an [AnyCommand][] contains multiple statements. |
...
An [AnyCommand][] parses a single statement provided in the [commandText][AnyCommand.CommandText], determining how the statement should be executed against the data source. If the [commandText][AnyCommand.CommandText] is a [Query Statement][Query Statements] the rows retrieved from the data source will be returned, otherwise if the [commandText][AnyCommand.CommandText] is a [Non Query Statement][Non Query Statements] the number of rows affected will be returned.
...
If multiple statements are provided in [CommandText][AnyCommand.CommandText], the block will throw a [CommandException][] as this type can only run single statements.

If performance is a key consideration it is recommended to use a [QueryCommand][Command Types QueryCommand] or [NonQueryCommand][Command Types NonQueryCommand] instead of [AnyCommand][] as they do not parse the [commandText][AnyCommand.CommandText].
