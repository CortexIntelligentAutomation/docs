...
description: "TODO: What is a flow execution."
weight: 1
---
...
A [flow][] can be debugged in [Cortex Studio][] by clicking the [Start an execution][] button or by making an [API call][API call: Cortex Studio] to [Cortex Studio][], [providing input variables][Providing Input Variables: Debugging] to the flow, if required; this will [validate the flow][Validating a Flow: Debugging] and then begin an execution.
...
- From [Cortex Studio][] using the [Inputs Property][] in the [Settings Tab][]
...
[Edit and Continue an Execution][] allows a developer to pause all executions, and make changes to the [flow][] before continuing debugging.
...
When debugging a [flow][] in [Cortex Studio][], [selecting an execution][Selecting an Execution] will display all initialised [variables][variable] that are in [scope][] in the [Variables Panel][].

To see the data in a variable, select the variable in the [Variables Panel][] and the data will be presented in the [Variable Details][] area below. If the data is large enough to negatively affect the performance of [Cortex Studio][] it will not be displayed unless the [Load value][] button is clicked.

### Viewing an Execution's Exceptions

When debugging a [flow][] in [Cortex Studio][], [selecting an execution][Selecting an Execution] will display all [exceptions][exception] that have been thrown for that execution in the [Exceptions Panel][].
...
Limitations:
    Edit and Continue:
        no way to update the variables value
    Set Next:
        set next only in the same workspace
        net next only works for single executions (may be fixed in future)
...
the easiest way to do this is by using the Set Variable block
...
[Validating a Flow: Debugging]: {{< ref "#flow-validation" >}}
...
[Edit and Continue]: {{< ref "#edit-and-continue-an-execution" >}}
...
[Validating a Flow: Production]: {{< ref "#providing-input-variables-1" >}}
...
[Exceptions Panel]: {{< url "Cortex.Guides.Studio.EastPanel.ExceptionsPanel" >}}
