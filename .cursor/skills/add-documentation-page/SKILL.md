...
- [ ] Cross-page and external links: keys reused or looked up in `data/urls.toml`; no plain `[…](https://…)`; no duplicate target URLs; footers dedupe by path with shared labels (exact case)
- [ ] Same-page links: `ref` footers (`#slug`), not inline `[…](#…)`; one def per anchor; exact-case labels / `[variant][canonical]`
- [ ] At most one new `urls.toml` key if this page is new; external keys only when the URL is new
