...
4. Look up keys in `data/urls.toml` (and sibling pages) before adding any. List files created/changed and any **new** `urls.toml` keys added (at most one per new page; never a second key for an existing page URL).
